...
def generate_data():
    ...
    try:
        with open(file_path, "w", encoding="utf-8") as f:
            ...
            f.write("TRUNCATE TABLE class_section_semesters RESTART IDENTITY CASCADE;\n")
            ...
            for i in range(1, len(department_names) + 1):
                ...
                f.write(f"INSERT INTO departments(department_id, title, description) VALUES ({sql_str(department['department_id'])}, {sql_str(department['title'])}, {sql_str(department['description'])});\n")
            ...
            for i in range(1, NUM_MAJORS + 1):
                ...
                majors.append(major)
                f.write(f"INSERT INTO majors(major_id, department_id, major_code, title, description) VALUES ({sql_str(major['major_id'])}, {sql_str(major['department_id'])}, {sql_str(major['major_code'])}, {sql_str(major['title'])}, {sql_str(major['description'])});\n")
            ...
            for i in range(1, NUM_PROFESSORS + 1):
                ...
                f.write(f"INSERT INTO professors(professor_sin, department_id, professor_email, first_name, last_name, birthday) VALUES ({sql_str(professor['professor_sin'])}, {sql_str(professor['department_id'])}, {sql_str(professor['professor_email'])}, {sql_str(professor['first_name'])}, {sql_str(professor['last_name'])}, {sql_str(professor['birthday'])});\n")
            ...
            for i in range(1, NUM_COURSES + 1):
                ...
                f.write(f"INSERT INTO courses(course_id, course_code, title, course_type, description) VALUES ({sql_str(course['course_id'])}, {sql_str(course['course_code'])}, {sql_str(course['title'])}, {sql_str(course['course_type'])}, {sql_str(course['description'])});\n")
            ...
            while cu_id <= NUM_COURSE_UNITS:
                ...
                if (course_id, major_id) not in used_cu_combinations:
                    ...
                    f.write(f"INSERT INTO course_units(course_unit_id, course_id, major_id, credit, required, gpa_requirement) VALUES ({sql_str(cu['course_unit_id'])}, {sql_str(cu['course_id'])}, {sql_str(cu['major_id'])}, {sql_str(cu['credit'])}, {sql_str(cu['required'])}, {sql_str(cu['gpa_requirement'])});\n")
                    cu_id += 1
            ...
            for i in range(1, NUM_SEMESTERS + 1):
                ...
                sem = {"semester_id": i, "name": f"Semester {i} - {start.year}", "start_date": start, "end_date": end}
                semesters.append(sem)
                f.write(f"INSERT INTO semesters(semester_id, name, start_date, end_date) VALUES ({sql_str(sem['semester_id'])}, {sql_str(sem['name'])}, {sql_str(sem['start_date'])}, {sql_str(sem['end_date'])});\n")
            ...
            while class_section_id <= NUM_CLASS_SECTIONS:
                ...
                no_gpa_course_units = [cu for cu in course_units if not cu["gpa_requirement"]]

                for data in no_gpa_course_units:
                    print(data["course_unit_id"])

                print(no_gpa_course_units)
                    
                cs = {
                    "class_section_semester_id": class_section_id,
                    "course_unit_id": random.choice(no_gpa_course_units)["course_unit_id"],
                ...
                f.write(f"INSERT INTO class_section_semesters(class_section_semester_id, course_unit_id, semester_id, professor_sin) VALUES ({sql_str(cs['class_section_semester_id'])}, {sql_str(cs['course_unit_id'])}, {sql_str(cs['semester_id'])}, {sql_str(cs['professor_sin'])});\n")
                class_section_id += 1
            f.write("SELECT setval('university.class_section_semesters_class_section_semester_id_seq', (SELECT MAX(class_section_semester_id) FROM university.class_section_semesters));\n")

            # --- 9. Timetables ---
            f.write("\n-- 9. Timetables\n")
            timetable_id = 1
            selected_class_sections = random.sample(class_sections, min(NUM_TIMETABLES, len(class_sections)))
            
            for cs in selected_class_sections:
                for j in range(random.randint(1, 2)):
                    if timetable_id > NUM_TIMETABLES: break
                    ...
                    f.write(f"INSERT INTO timetables(timetables_id, class_section_semester_id, room, status, schedule_time) VALUES ({sql_str(timetable['timetables_id'])}, {sql_str(timetable['class_section_semester_id'])}, {sql_str(timetable['room'])}, {sql_str(timetable['status'])}, {sql_str(timetable['schedule_time'])});\n")
                    timetable_id += 1

            # --- 10. Student Enrollments ---
            cu_gpa_lookup = {cu["course_unit_id"]: cu["gpa_requirement"] for cu in course_units}

            eligible_class_sections = [
                cs for cs in class_sections 
                if cu_gpa_lookup.get(cs['course_unit_id']) is None
            ]

            f.write("\n-- 10. Student Enrollments\n")
            used_enrollments = set()
            enrollment_id = 1
            list_of_students = students[:50]
            while enrollment_id <= NUM_ENROLLMENTS:
                student_id = random.choice(list_of_students)["student_id"]

                if not class_sections: break
                class_section_semester_id = random.choice(eligible_class_sections)["class_section_semester_id"]
                ...
                if (student_id, class_section_semester_id) not in used_enrollments:
                    ...
                    enrollment_id += 1
            f.write("SELECT setval('university.student_enrollments_student_enrollment_id_seq', (SELECT MAX(student_enrollment_id) FROM university.student_enrollments));\n")


            # --- 11. Prerequisites (Check unique combination of course_id and prerequisite_course_id) ---
            f.write("\n-- 11. Prerequisites\n")
            prereq_count = 0
            while prereq_count < NUM_PREREQUISITES:
                ...
                if course["course_type"] != "General" and course_id != prereq_id:
                    ...
                    if pair not in prerequisites:
                        ...
                        f.write(f"INSERT INTO prerequisites(course_id, prerequisite_course_id) VALUES ({sql_str(course_id)}, {sql_str(prereq_id)});\n")
                        prereq_count += 1
